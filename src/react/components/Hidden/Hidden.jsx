...
export default function Hidden({ sm, md, lg, xl, xxl, hover, active, focus, all=true, children, ...props }) {
    const states = Object.entries({ all, sm, md, lg, xl, xxl, hover, active, focus }).map(([key, value]) => {
    ...
    return (
        <div className={`${statesRaw[states.sm]} ${statesRaw[states.md]} ${statesRaw[states.lg]} ${statesRaw[states.xl]} ${statesRaw[states.xxl]} ${statesRaw[states.hover]} ${statesRaw[states.active]} ${statesRaw[states.focus]} ${statesRaw[states.all]}`} {...props}>
            {children}
        </div>
    );
}
